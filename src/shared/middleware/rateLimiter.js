...
/**
 * Redis-backed fixed-window rate limiter (rules/05). Keyed by authenticated user
 * id when available, else client IP. Fails OPEN on Redis errors so an infra blip
 * never locks everyone out, but always enforces the limit when Redis is healthy.
 *
 * @param {{ key?: string, max?: number, windowSec?: number }} options
 */
